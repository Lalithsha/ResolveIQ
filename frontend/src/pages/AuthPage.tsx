  ...
  return <main className="min-h-screen bg-background grid lg:grid-cols-2">
    ...
    <section className="flex items-center justify-center p-6">
      <form onSubmit={submit} className="w-full max-w-md bg-surface border border-border rounded-card p-8 shadow-sm space-y-5">
        ...
        {mode === 'register' && <label className="block text-sm font-medium">Full name<input className="mt-1 w-full h-11 px-3 bg-surface text-[var(--color-text)] border border-border rounded-input focus:outline-none focus:ring-2 focus:ring-primary" value={fullName} onChange={e => setFullName(e.target.value)} required /></label>}
        <label className="block text-sm font-medium">Email<input type="email" className="mt-1 w-full h-11 px-3 bg-surface text-[var(--color-text)] border border-border rounded-input focus:outline-none focus:ring-2 focus:ring-primary" value={email} onChange={e => setEmail(e.target.value)} required /></label>
        <label className="block text-sm font-medium">Password<input type="password" minLength={12} maxLength={128} className="mt-1 w-full h-11 px-3 bg-surface text-[var(--color-text)] border border-border rounded-input focus:outline-none focus:ring-2 focus:ring-primary" value={password} onChange={e => setPassword(e.target.value)} required /></label>
