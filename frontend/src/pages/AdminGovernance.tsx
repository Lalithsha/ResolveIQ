...
const Overview: React.FC<OverviewProps> = props => {
  ...
  return <><section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{cards.map(([label, value, Icon, tone]) => <article key={label} className="panel p-4"><div className="flex justify-between text-xs text-muted"><span>{label}</span><Icon className={`h-4 w-4 ${tone}`} /></div><strong className="mt-4 block text-2xl">{value}</strong></article>)}</section><Workflows workflows={props.workflows} readOnly={props.readOnly} onRetry={props.onRetry} retryingId={props.retryingId} /></>;
};
